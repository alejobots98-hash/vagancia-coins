...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.startsWith('!wcoin')) {
        ...
        try {
            ...
            if (!user) {
                user = await createUser(member.id);
            }

            user.coins = parseFloat(
                (user.coins + 0.15).toFixed(2)
            );
    ...
    if (message.content.startsWith('!resetcoin')) {
        ...
        if (!member) {
            return message.reply(
                '❌ Uso correcto: `!resetcoin @usuario`'
            );
        }
        ...
        try {
            ...
            return message.reply(
                `${VG_EMOJI} Monedas reseteadas correctamente`
            );
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (user.coins < reward.coins) {
        ...
        return interaction.editReply({
            content:
                `❌ Necesitás ${reward.coins} VG COINS`
        });
    }
