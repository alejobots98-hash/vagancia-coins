    ...
    if (message.content === '!topcoins') {
        try {
            ...
            try {
                ...
                ctx.drawImage(coin, 655, 415, 105, 105);
                ctx.restore();
            } catch (e) {}
...
${VG_EMOJI} Premio: \`${reward.name}\`
${VG_EMOJI} Costo: \`${reward.coins} VG COINS\`
${VG_EMOJI} Saldo restante: \`${(user.coins - reward.coins).toFixed(2)} VG\`
`);
        ...
        await ticketChannel.send({ content: `<@&${STAFF_ROLE_ID}> Nuevo reclamo.`, embeds: [embed], components: [row], files: [file] });
